<template>
  <div class="p-2 text-white bg-gray-600">
    <label for="profileUpload">프로필 수정</label>
    <div class="flex">
    ...
    <div class="mb-4">
      ...
      <input class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline" type="text" id="username" v-model="email">
      <div v-if="errorMessage" class="mb-3 text-red-600">{{ errorMessage }}</div>
      <button @click="emailUpdate" class="m-2 ml-auto btn btn-blue">수정</button>
    </div>
  </div>
</template>
